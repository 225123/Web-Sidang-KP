...
class NotifikasiController extends Controller
{
        ...
        $query = NotifikasiLog::with('sender')
            ->where(function ($q) use ($user) {
                $q->where('receiver_id', $user->id)
                    ->orWhere(function ($sq) {
                        $sq->whereIn('target_role', ['dosen', 'semua'])
    ...
    public function show($id)
    {
        ...
        if ($notifikasi->receiver_id == auth()->id() || $notifikasi->target_role == 'dosen' || $notifikasi->target_role == 'semua') {
            $notifikasi->update(['is_read' => true]);
        }

        return view('dosen.notifikasi-detail', compact('notifikasi'));
    }
    ...
    public function redirect($id)
    {
        ...
        return redirect()->route('dosen.notifikasi.show', $notifikasi->id);
    }
}
