...
@php
    ...
    $activeClass = "flex items-center gap-3 px-6 h-[35px] mx-[6px] text-[13px] font-medium text-white bg-[#4CC098] rounded-[5px] transition-colors cursor-pointer text-left overflow-hidden whitespace-nowrap w-full max-w-[207px]";
    ...
    $subActiveClass = "flex items-center gap-3 pl-10 pr-4 py-1.5 text-[12px] font-medium text-white transition-colors bg-[#4CC098] w-full rounded-none whitespace-nowrap";
@endphp
...
<div x-data="{ open: localStorage.getItem('koor_pendaftaran') === 'true' }" class="mt-1">
    <button @click="open = !open; localStorage.setItem('koor_pendaftaran', open)" class="{{ $inactiveClass }} flex justify-between items-center outline-none">
...
<div x-data="{ open: localStorage.getItem('koor_bimbingan') === 'true' }" class="mt-1">
    <button @click="open = !open; localStorage.setItem('koor_bimbingan', open)" class="{{ $inactiveClass }} flex justify-between items-center outline-none">
        <div class="flex items-center gap-3">
    ...
    <div x-show="open" x-transition class="mt-0.5 bg-[#BBB8B8] w-full overflow-hidden flex flex-col divide-y divide-[#9E9B9B]">
        ...
        <a href="{{ route('koordinator.dummy', 'persetujuan-sidang') }}" class="{{ $active == 'persetujuan-sidang' ? $subActiveClass : $subInactiveClass }}">
...
<div x-data="{ open: localStorage.getItem('koor_manajemen_sidang') === 'true' }" class="mt-1">
    <button @click="open = !open; localStorage.setItem('koor_manajemen_sidang', open)" class="{{ $inactiveClass }} flex justify-between items-center outline-none">
        <div class="flex items-center gap-3">
    ...
    <div x-show="open" x-transition class="mt-0.5 bg-[#BBB8B8] w-full overflow-hidden flex flex-col divide-y divide-[#9E9B9B]">
        ...
        <a href="{{ route('koordinator.dummy', 'penjadwalan-sidang') }}" class="{{ $active == 'penjadwalan-sidang' ? $subActiveClass : $subInactiveClass }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            <span x-show="sidebarOpen" x-transition>Penjadwalan Sidang</span>
        </a>
        <a href="{{ route('koordinator.dummy', 'dosen-penguji') }}" class="{{ $active == 'dosen-penguji' ? $subActiveClass : $subInactiveClass }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
            <span x-show="sidebarOpen" x-transition>Dosen Penguji</span>
        </a>
        <a href="{{ route('koordinator.dummy', 'kalender-sidang') }}" class="{{ $active == 'kalender-sidang' ? $subActiveClass : $subInactiveClass }}">
...
<div x-data="{ open: localStorage.getItem('koor_penilaian') === 'true' }" class="mt-1">
    <button @click="open = !open; localStorage.setItem('koor_penilaian', open)" class="{{ $inactiveClass }} flex justify-between items-center outline-none">
        <div class="flex items-center gap-3">
    ...
    <div x-show="open" x-transition class="mt-0.5 bg-[#BBB8B8] w-full overflow-hidden flex flex-col divide-y divide-[#9E9B9B]">
        <a href="{{ route('koordinator.dummy', 'jadwal-menguji') }}" class="{{ $active == 'jadwal-menguji' ? $subActiveClass : $subInactiveClass }}">
        ...
        <a href="{{ route('koordinator.dummy', 'input-nilai') }}" class="{{ $active == 'input-nilai' ? $subActiveClass : $subInactiveClass }}">
...
<div x-data="{ open: localStorage.getItem('koor_pasca_sidang') === 'true' }" class="mt-1">
    <button @click="open = !open; localStorage.setItem('koor_pasca_sidang', open)" class="{{ $inactiveClass }} flex justify-between items-center outline-none">
        <div class="flex items-center gap-3">
    ...
    <div x-show="open" x-transition class="mt-0.5 bg-[#BBB8B8] w-full overflow-hidden flex flex-col divide-y divide-[#9E9B9B]">
        ...
        <a href="{{ route('koordinator.dummy', 'laporan-arsip') }}" class="{{ $active == 'laporan-arsip' ? $subActiveClass : $subInactiveClass }}">
...
<div x-data="{ open: localStorage.getItem('koor_sistem') === 'true' }" class="mt-1">
    <button @click="open = !open; localStorage.setItem('koor_sistem', open)" class="{{ $inactiveClass }} flex justify-between items-center outline-none">
        <div class="flex items-center gap-3">
    ...
    <div x-show="open" x-transition class="mt-0.5 bg-[#BBB8B8] w-full overflow-hidden flex flex-col divide-y divide-[#9E9B9B]">
        ...
        <a href="{{ route('koordinator.dummy', 'manajemen-akses') }}" class="{{ $active == 'manajemen-akses' ? $subActiveClass : $subInactiveClass }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
            <span x-show="sidebarOpen" x-transition>Manajemen Akses</span>
        </a>
...
<div class="mt-4">
    ...
    <div class="mt-1 pb-4">
        ...
        <a href="#" onclick="event.preventDefault(); document.getElementById('sidebar-logout-form-koor').submit();" class="flex items-center gap-3 px-6 h-[35px] mx-[1px] w-full max-w-[219px] overflow-hidden whitespace-nowrap rounded-[5px] text-[12px] font-bold text-[#FF0000] hover:bg-[#E8E5E5] transition-colors cursor-pointer text-left">
