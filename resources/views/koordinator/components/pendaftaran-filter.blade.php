...
<div class="mb-4 flex flex-col lg:flex-row justify-between items-center gap-4 w-full">
    ...
    <div class="flex flex-wrap items-center gap-4 w-full lg:w-auto justify-start lg:justify-end">
        <div class="flex items-center gap-2 w-full sm:w-auto" x-data="{ openJenis: false, selectedJenis: '{{ request($prefix.'.jenis_kp', 'All') }}' }" @reset-dropdowns-{{ $prefix }}.window="selectedJenis = 'All'">
            ...
            <div class="relative w-full sm:w-[130px]">
                ...
                <div x-show="openJenis" x-transition style="display: none;" class="absolute z-50 w-full mt-1 bg-white border border-[#CAC0C0] rounded shadow-lg overflow-hidden">
                    <ul class="py-1 text-[13px]">
                        ...
                        <li><label class="block px-3 py-1.5 hover:bg-gray-100 cursor-pointer"><input type="radio" name="{{ $prefix }}[jenis_kp]" value="External" class="hidden" x-model="selectedJenis" @change="$dispatch('update-jenis-' + '{{ $prefix }}', selectedJenis)">External</label></li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        @if(!$hideStatus)
        <div class="flex items-center gap-2 w-full sm:w-auto mt-2 sm:mt-0" x-data="{ openStatus: false, selectedStatus: '{{ request($prefix.'.status_approval', 'All') }}' }" @reset-dropdowns-{{ $prefix }}.window="selectedStatus = 'All'">
        ...
        <div class="flex items-center gap-2 w-full sm:w-auto mt-2 sm:mt-0" x-data="{ openPengerjaan: false, selectedPengerjaan: '{{ request($prefix.'.pengerjaan', 'All') }}' }" @reset-dropdowns-{{ $prefix }}.window="selectedPengerjaan = 'All'">
        ...
        <div class="flex gap-2 w-full sm:w-auto mt-2 sm:mt-0">
            ...
            @if($prefix === 'main')
                <button type="button" @click="isSelectionMode = !isSelectionMode; sessionStorage.setItem('kpSelectionMode', isSelectionMode)" class="flex-1 sm:flex-none bg-[#4285F4] hover:bg-blue-600 transition-colors text-white px-4 py-1.5 rounded-[5px] text-[12px] font-bold shadow-sm whitespace-nowrap flex items-center justify-center" x-text="isSelectionMode ? 'Batal Pilih' : 'Pilih Beberapa'"></button>
            @endif
        </div>
    </div>
</div>
