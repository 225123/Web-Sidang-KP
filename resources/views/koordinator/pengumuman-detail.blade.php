<x-dashboard-layout header="" userName="{{ auth()->user()->name }}" roleName="KOORDINATOR KP" hidePeriodSelector="true">
    <x-slot:sidebar>
        ...
        <div class="mt-8 px-4 w-full">
            <div class="max-w-5xl mx-auto">
                ...
                <div class="flex items-center gap-4 mb-6">
                    ...
                    <h2 class="text-[22px] font-bold text-black border-none m-0">Detail Pengumuman</h2>
                </div>

                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-12">
                    <div class="px-8 py-10">

                        <!-- 1. Sender Info Row (Me) -->
                        <div class="flex items-start justify-between mb-3 border-b border-gray-50 pb-3">
                            <div class="flex items-center gap-4">
                                <!-- Avatar -->
                                <div
                                    class="w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-[18px] shadow-sm">
                                    {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                                </div>
                                <div class="flex flex-col">
                                    <div class="flex items-center gap-2">
                                        <span class="font-bold text-[15px] text-[#202124]">{{ auth()->user()->name }}</span>
                                        <span class="text-[13px] text-[#5f6368]">&lt;{{ auth()->user()->email }}&gt;</span>
                                    </div>
                            ...
                            <div class="text-[13px] text-[#5f6368] font-medium">
                                {{ $log->created_at->isoFormat('DD MMM YYYY, HH:mm') }}
                        ...
                        <!-- 3. Content Area -->
                        <div class="text-[15px] text-black leading-relaxed whitespace-pre-wrap min-h-[150px] font-sans">{{ $log->pesan }}</div>
                            ...
                            <div class="mt-6 pt-4 border-t border-gray-100">
                                ...
                                <div
                                    ...
                                    <div
                                        ...
                                        @php
                                            $ext = pathinfo($log->file_path, PATHINFO_EXTENSION);
