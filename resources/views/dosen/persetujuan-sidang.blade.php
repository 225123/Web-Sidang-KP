        ...
        <div class="mt-6" x-data="{ showTolakModal: false, selectedId: null }">
            ...
            <div class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                <div class="overflow-x-auto">
                    <table class="w-full border-collapse text-sm">
                        ...
                        <tbody class="text-center text-gray-800">
                            @php $aktifPengajuans = $pengajuans->whereIn('status_verifikasi', ['pending', 'verified']); @endphp
                            @forelse($aktifPengajuans as $index => $pengajuan)
                                    ...
                                    <td class="py-3 px-4 border-r border-gray-200">
                                        @if($pengajuan->status_verifikasi == 'pending')
                                            <div class="flex items-center justify-center gap-2">
                                                <form action="{{ route('dosen.persetujuan-sidang.update', $pengajuan->id) }}"
                                                    method="POST">
                                                    @csrf @method('PUT')
                                                    <button type="submit"
                ...
                <div class="p-4 flex justify-between items-center bg-white border-t border-gray-200">
                    <span class="text-xs text-gray-500 font-medium">1 - {{ count($aktifPengajuans ?? []) }} of
                        {{ count($aktifPengajuans ?? []) }} entries</span>
                    <div class="flex gap-1 border border-gray-200 rounded p-0.5 bg-white shadow-sm text-sm">
                        <button class="px-2 py-1 text-gray-400 bg-transparent disabled:opacity-50 hover:bg-gray-50 rounded" disabled><</button>
                        <button class="px-2.5 py-1 text-white bg-[#3B5998] rounded">1</button>
                        <button class="px-2.5 py-1 text-gray-600 bg-transparent hover:bg-gray-50 rounded">></button>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 mb-4">
                ...
                <p class="text-sm text-gray-600">Daftar mahasiswa yang pengajuannya telah dikembalikan untuk diperbaiki beserta catatannya.</p>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm mb-6">
                <div class="overflow-x-auto">
                    <table class="w-full border-collapse text-sm">
                        <thead>
                            <tr class="bg-[#FFEAEA] text-gray-700 border-b border-gray-300">
                                <th class="py-3 px-4 font-semibold text-center w-[5%] border-r border-gray-300">No</th>
                                <th class="py-3 px-4 font-semibold text-center border-r border-gray-300">Mahasiswa</th>
                                <th class="py-3 px-4 font-semibold text-center border-r border-gray-300">Judul KP</th>
                                <th class="py-3 px-4 font-semibold text-center border-r border-gray-300 w-[20%]">Laporan KP</th>
                                <th class="py-3 px-4 font-semibold text-center border-r border-gray-300">Catatan Revisi</th>
                                <th class="py-3 px-4 font-semibold text-center">Log Bimbingan</th>
                            </tr>
                        </thead>
                        <tbody class="text-center text-gray-800">
                            @php $ditolakPengajuans = $pengajuans->where('status_verifikasi', 'rejected'); @endphp
                            @forelse($ditolakPengajuans as $index => $pengajuan)
                                <tr class="border-b border-gray-200 hover:bg-gray-50 h-[45px]">
                                    <td class="py-3 px-4 border-r border-gray-200">{{ $loop->iteration }}</td>
                                    <td class="py-3 px-4 border-r border-gray-200 text-left">
                                        <div class="font-bold">{{ $pengajuan->mahasiswa?->user?->name ?? 'User' }}</div>
                                        <div class="text-gray-500 text-[11px]">{{ $pengajuan->mahasiswa?->nim ?? '-' }}</div>
                                    </td>
                                    <td class="py-3 px-4 border-r border-gray-200 text-[13px]">{{ $pengajuan->pendaftaranKp?->judul_kp ?? '-' }}</td>
                                    <td class="py-3 px-4 border-r border-gray-200 text-[12px] text-blue-600">
                                        @if($pengajuan->file_laporan)
                                            <a href="{{ asset('storage/' . $pengajuan->file_laporan) }}" target="_blank" class="hover:underline font-medium italic">Lihat Dokumen</a>
                                        @elseif($pengajuan->link_github)
                                            <a href="{{ $pengajuan->link_github }}" target="_blank" class="hover:underline font-medium italic">Buka GDrive</a>
                                        @endif
                                    </td>
                                    <td class="py-3 px-4 border-r border-gray-200 text-left text-[12px] text-red-600 italic">
                                        "{{ $pengajuan->dosen_feedback ?? 'Tidak ada catatan.' }}"
                                    </td>
                                    <td class="py-3 px-4">
                                        <a href="#" class="bg-[#3B5998] text-white text-[11px] font-semibold px-4 py-1.5 rounded-full inline-block">Detail</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="py-10 text-gray-400 italic">Belum ada riwayat penolakan.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="p-4 flex justify-between items-center bg-white border-t border-gray-200">
                    <span class="text-xs text-gray-500 font-medium">1 - {{ count($ditolakPengajuans ?? []) }} of
                        {{ count($ditolakPengajuans ?? []) }} entries</span>
                    <div class="flex gap-1 border border-gray-200 rounded p-0.5 bg-white shadow-sm text-sm">
                        <button class="px-2 py-1 text-gray-400 bg-transparent disabled:opacity-50 hover:bg-gray-50 rounded" disabled><</button>
                        <button class="px-2.5 py-1 text-white bg-[#3B5998] rounded">1</button>
                        <button class="px-2.5 py-1 text-gray-600 bg-transparent hover:bg-gray-50 rounded">></button>
            ...
            <div x-show="showTolakModal" style="display:none;"
                ...
                <div @click.away="showTolakModal = false"
                    ...
                    <form :action="'{{ url('dosen/persetujuan-sidang') }}/' + selectedId + '/tolak'" method="POST"
                        class="p-6">
                        @csrf
                        @method('DELETE')
                        <p class="text-sm text-gray-700 mb-4">Berikan alasan penolakan agar mahasiswa dapat memperbaiki
                            laporannya. Data pengajuan ini akan dikembalikan ke Riwayat Penolakan.</p>
