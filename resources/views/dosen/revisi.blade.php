            ...
            <div x-data="{ open: false, selected: 'Genap 2025/2026' }" class="relative w-[212px]">
                <button @click="open = !open" @click.outside="open = false" type="button"
                    ...
                    <svg :class="open ? 'rotate-0' : 'rotate-90'"
                        class="w-3.5 h-3.5 text-gray-500 transition-all duration-200" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    </svg>
                </button>
                <div x-show="open" x-transition x-cloak
                    class="absolute z-50 w-full mt-1 bg-[#FBFBFB] border border-[#CAC0C0] rounded-[5px] shadow-lg">
                    <ul class="py-1 text-[13px] font-medium">
                        <li><button @click="selected = 'Genap 2025/2026'; open = false"
                                class="block w-full text-left px-3 py-2 hover:bg-[#E8E5E5]">Genap 2025/2026</button>
                        </li>
                        <li><button @click="selected = 'Ganjil 2025/2026'; open = false"
                                class="block w-full text-left px-3 py-2 hover:bg-[#E8E5E5]">Ganjil 2025/2026</button>
                        </li>
                    </ul>
                </div>
            </div>
                                            ...
                                            <template x-if="sidang.status_revisi === 'Menunggu'">
                                                <div class="flex gap-2">
                                                    <form :action="'{{ url('dosen/revisi') }}/' + sidang.id + '/terima'"
                                                        ...
                                                        @csrf
                                                        <button type="submit"
                                                            class="bg-[#34A853] hover:bg-green-700 text-white font-bold text-[12px] px-3 py-1.5 rounded-[4px] shadow-sm transition-colors uppercase">Sahkan</button>
                                                    </form>
