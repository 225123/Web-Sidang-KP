<x-dashboard-layout header="" userName="{{ auth()->user()->name }}" roleName="MAHASISWA" hidePeriodSelector="true">
    <x-slot:sidebar>
                    ...
                    <div class="px-8 py-10">
                        ...
                        <div class="flex items-start justify-between mb-3 border-b border-gray-50 pb-3">
                            <div class="flex items-center gap-4">
                                <!-- Avatar -->
                                <div
                                    class="w-12 h-12 rounded-full bg-orange-600 flex items-center justify-center text-white font-bold text-[18px] shadow-sm">
                                    {{ strtoupper(substr($notifikasi->sender->name ?? 'S', 0, 1)) }}
                                </div>
                                <div class="flex flex-col">
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="font-bold text-[15px] text-[#202124]">{{ $notifikasi->sender->name ?? 'Sistem' }}</span>
                                        <span class="text-[13px] text-[#5f6368]">&lt;{{ $notifikasi->sender->email ??
                                            'noreply@sistem.com' }}&gt;</span>
                                    </div>
                            ...
                            <div class="text-[13px] text-[#5f6368] font-medium">
                                {{ $notifikasi->created_at->isoFormat('DD MMM YYYY, HH:mm') }}
                        ...
                        @if($notifikasi->target_url)
                            <div class="mt-10 mb-2">
                                <a href="{{ route('mahasiswa.notifikasi.redirect', $notifikasi->id) }}" 
                        ...
                        @if($notifikasi->file_path)
                            <div class="mt-6 pt-4 border-t border-gray-100">
                                <p
                                    class="text-[14px] font-bold text-[#202124] mb-5 flex items-center gap-2 uppercase tracking-wide">
                                ...
                                <!-- Attachment Card -->
                                <div
                                    class="group relative w-[240px] bg-[#f8f9fa] rounded-lg border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300">
                                    <!-- Thumbnail Area -->
                                    <div
                                        class="h-[140px] bg-white flex items-center justify-center border-b border-gray-100 overflow-hidden">
                                        @php
                                        ...
                                        @if($isImage)
                                            <img src="{{ asset('storage/' . $notifikasi->file_path) }}"
                                        ...
                                        <div
                                            ...
                                            <a href="{{ asset('storage/' . $notifikasi->file_path) }}" target="_blank"
                                            ...
                                            <a href="{{ asset('storage/' . $notifikasi->file_path) }}" download
                                                class="p-3 bg-white/20 hover:bg-white/40 rounded-full text-white transition-all transform hover:scale-110"
                                                title="Download">
                                                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div class="px-4 py-3 flex items-center gap-3 bg-white">
                                        ...
                                        <span class="text-[12px] font-bold text-[#202124] truncate"
                                            title="{{ basename($notifikasi->file_path) }}">
