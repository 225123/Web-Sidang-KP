...
// Extract parts
$partStart = substr($content, 0, strpos($content, '<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">'));
$partEnd = substr($content, strpos($content, '<script>'));

// We need to extract the inner HTML of the components
...
$chartStr = extractDiv($content, $chartStart, '<!-- Timeline Card -->');
$chartStr = str_replace('lg:col-span-2 ', '', $chartStr); // remove lg:col-span-2
$chartStr = trim(preg_replace('/<!-- Timeline Card -->.*/s', '', $chartStr));
// The end of chartStr might need exact matching of div. Since it's right before Timeline Card, we just cut at `</div>` before `<!-- Timeline Card -->`.
...
$chartStr = preg_replace('/^<div class="lg:col-span-2 /', '<div class="', $chartStr);

// Progress Sidang
$progSidangStart = '<!-- Progress Chart -->';
$progSidangStr = substr($content, strpos($content, $progSidangStart), strpos($content, '<!-- Notifikasi -->') - strpos($content, $progSidangStart));
$progSidangStr = trim($progSidangStr);
$progSidangStr = str_replace('<div class="lg:col-span-2 ', '<div class="', $progSidangStr);
...
// wait, we just want up to the end of its div.
$persetujuanStr = preg_replace('/<\/div>\s*<\/div>\s*<\/div>\s*$/s', '</div>', $persetujuanStr); // remove outer grid endings
$persetujuanStr = str_replace('<div class="lg:col-span-2 ', '<div class="', $persetujuanStr);

// Notifikasi
$notifStart = '<!-- Notifikasi -->';
...
// New Timeline & Jadwal Sidang based on Dosen
$timelineJadwal = <<<HTML
...
echo "Dashboard rewritten successfully!";
