...
class DummyKpSeeder extends Seeder
{
    public function run()
    {
        ...
        $ta = \Illuminate\Support\Facades\DB::table('tahun_ajaran')->where('tahun', '2025')->first();
        if (!$ta) {
            $taId = \Illuminate\Support\Facades\DB::table('tahun_ajaran')->insertGetId([
                'tahun' => '2025',
                'label_tahun_ajaran' => '2025/2026',
                'is_active' => true,
                'semester' => 'Genap'
            ]);
            $ta = \Illuminate\Support\Facades\DB::table('tahun_ajaran')->find($taId);
        }

        $mhsIndex = 0;
        foreach($configs as $pIndex => $groupSize) {
            if ($mhsIndex >= count($mahasiswas)) break;
